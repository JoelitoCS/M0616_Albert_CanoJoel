...
<h1>Usuarios</h1>
<a class="add-btn" href="addUser.php">+ Añadir Usuario</a>
    ...
    <table>
        ...
        <?php foreach ($user as $u): ?>
            <tr>
                ...
                <td><?= $u['email'] ?></td>
                <td><?= $u['password'] ?></td>
                <td><img style="width: 50px; height: 50px;" src="<?= $u['foto'] ?>" alt="👤"></td>
                <td><?= $u['role'] ?></td>
                <td>
                    <a href="editUser.php?id=<?= $u['id'] ?>">Editar</a> |
                    <a onclick="return confirm('¿Seguro que quieres eliminarlo?')" href="deleteUser.php?id=<?= $u['id'] ?>">Eliminar</a>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
